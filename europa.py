...
def write_europa_csv(outfile, results):
    outfile.write("home, homescore, awayscore, away, date, stage\n")
    for result in results:
        outfile.write(u'{0}, {1}, {2}, {3}, "{4}", "{5}"\n'.format(*result))

def parse_europa_page(soup):
...
def europa():
    url = "http://espnfc.com/results/_/league/uefa.europa/uefa-europa-league"
    ...
    results = parse_europa_page(soup)
    ...
    while 1:
        ...
        if soup.h2.text.split(" ")[0] != season:
            ...
            out = codecs.open("data/europa_{0}.csv".format(season_f), 'w', "utf8")
            write_europa_csv(out, results)
